...
From the S entrance, step 1N and then start going W. You'll find a stone that marks the grave of Zaton, the master of Druid Magic. Remember this place; we'll come back to it later.
...
E of the tavern there's a spot where the walls have crumbled and you can `(U)se` *Climb* to get through. Save here, and use Dragon Stones if you need them, because the upcoming battle is tough. Circle around and find the Mud Toad Militia `(p32)`. You learn an important fact about Lanac'toor, which is that he was turned to stone when the forces of Kingshome destroyed the city. So that's not just a *statue*, it's the (petrified) man himself. Combined with the hint we got back on the [first Guard Bridge](#guard-bridge-1), it seems like we could put him back together if we can find all of the stone body parts. Fortunately, I told you about this already when we found [Tars](#shame-if-anything-were-to-happen-to-it-tars-ruins) back on Forlorn, so I guess you could say that forewarned is... forearmed! [puts on sunglasses]

As a side note, we now know who all three Master Mages of Dilmun are: Zaton the Druid, Mystalvision the Sun Mage, and Lanac'toor the High Mage. Mystalvision is clearly in cahoots with Namtar, and the other two are dead.
